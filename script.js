...
}, { threshold: 0.5 }); // Define que o elemento deve estar 50% visível para ser considerado visível

// Seleciona todos os títulos, subtítulos e projetos e começa a observar
document.querySelectorAll('.titulo-container, .subtitle, .project').forEach(element => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const handleScroll = () => {
        const triggerPoint = window.innerHeight / 1; // Ponto de ativação
        const currentScrollY = window.scrollY;

        animatedElements.forEach((el) => {
            const rect = el.getBoundingClientRect();
            const isBelow = rect.top > window.innerHeight / 3; // Elemento saiu para baixo
            const isVisible = rect.top < triggerPoint && rect.bottom >= 0;

            // Verifica se rolou para baixo e o elemento está visível
            if (currentScrollY > lastScrollY && isVisible) {
                el.classList.add("show");
            }

            // Verifica se rolou para cima e o elemento saiu para baixo
...
document.addEventListener('DOMContentLoaded', () => {
    const animatedElementsContato = document.querySelectorAll(".animated-group-contato");
    let lastScrollY = window.scrollY;
